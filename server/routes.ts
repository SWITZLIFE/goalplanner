...
Write it in a conversational tone, like you're talking to a friend.`;
    ...
    try {
      ...
      if (remainingTasks.length > 0) {
        ...
        await db.update(goals)
          .set({ progress: 0 })
          .where(and(
            eq(goals.id,goals.id, taskToDelete.goalId),
            eq(goals.userId, userId)
          ));
      }
  ...
  app.post("/api/forum/posts/:postId/comments", requireAuth, async (req, res) => {
    try {
      ...
      // Create initial upvote from the author
      await db.insert(forumReactions)
        .values({
          userId,
  ...
  // Comment voting API
  app.post("/api/forum/comments/:commentId/vote", requireAuth, async (req, res) => {
    try {
      ...
      const existingVote = await db.select()
        ...
        .where(and(
          ...
          eq(forumReactions.commentId, parseInt(commentId))
        ))
        .limit(1);

      if (existingVote.length > 0) {
        // If vote type is same as existing, remove the vote
      ...
      const downvotes = await db.select({ count: sql<number>`count(*)` })
        ...
        .where(and(
          eq(forumReactions.commentId, parseInt(commentId)),
          eq(forumReactions.type, 'downvote')
        ));

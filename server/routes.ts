...
- Make it feel like a cozy conversation, not a formal letter`;
 ...
 app.get("/api/analytics/coin-history", requireAuth, async (req, res) => {
   try {
     const userId = req.user!.id;

     // Get the last 30 days of coin history
     const coinHistory = await db.select({
       date: sql`DATE(created_at)::text`,
       coins: sql`SUM(coins_earned)`
     })
       .from(timeTracking)
       .where(and(
         eq(timeTracking.userId, userId),
 ...
 app.get("/api/analytics/leaderboard", requireAuth, async (req, res) => {
   try {
     const leaderboard = await db.select({
       ...
       totalCoins: sql`COALESCE(SUM(${timeTracking.coinsEarned}), 0)::integer`
     })
       .from(users)
       .leftJoin(timeTracking, eq(users.id, timeTracking.userId))
       .groupBy(users.id, users.username)
       .orderBy(sql`COALESCE(SUM(${timeTracking.coinsEarned}), 0) DESC`)
       .limit(10);
 ...
 app.get("/api/analytics/time-tracking", requireAuth, async (req, res) => {
   try {
     const userId = req.user!.id;

     // Get the last 30 days of time tracking data
     const timeTrackingData = await db.select({
       date: sql`DATE(start_time)::text`,
       totalMinutes: sql`SUM(
       ...
       .where(and(
         eq(timeTracking.userId, userId),
         sql`start_time >= NOW() - INTERVAL '30 days'`,
         isNull(timeTracking.isActive)
       ))
